...
local APPLICANT_LIST_HEADER = {
  {
    ...
    iconHandler = function(applicant)
      ...
      if not regimentData then
        return
      end

      if regimentData.isNewbie and regimentData.newbieExpireTime and time() < regimentData.newbieExpireTime then
        return format([[Interface\AddOns\MeetingStone\Media\Locomotive\Newbie]]), 0, 0.75, 0, 0.75
      end

      -- 大秘境逻辑
      if activityInfo.displayType == Enum.LFGListDisplayType.RoleEnumerate then
        if regimentData.level == nil or regimentData.level == -1 then
          return
        end
        return format([[Interface\AddOns\MeetingStone\Media\Locomotive\Locomotive%d]], regimentData.level), 0, 0.75, 0, 0.75
      end

      if regimentData.starLevel == nil or regimentData.starLevel == -1 then
        return
      end
      return format([[Interface\AddOns\MeetingStone\Media\certification_icon\certification_icon_%d]], 5), 0, 0.96, 0, 0.42
    end
  },
  {
    key = 'Icon',
...
function ApplicantPanel:OnInitialize()
  ...
  -- 收到服务端战团/萌新数据后，刷新列表使最新信息同步到 UI
  self:RegisterMessage('MEETINGSTONE_SQGLIB_UPDATED', function()
    self:UpdateApplicantsList()
  end)
